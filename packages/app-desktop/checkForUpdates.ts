...
import {
  ...
  extractVersionInfo,
  GitHubRelease,
} from "./utils/checkForUpdatesUtils";
...
async function fetchLatestReleases() {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Cannot get latest release info: ${responseText.substr(0, 500)}`
    );
  }
...
async function getSkippedVersions(): Promise<string[]> {
  const r = await KvStore.instance().value<string>(
    "updateCheck::skippedVersions"
  );
...
async function addSkippedVersion(s: string) {
  ...
  await KvStore.instance().setValue(
    "updateCheck::skippedVersions",
...
export default async function checkForUpdates(
  ...
  try {
    ...
    const release = extractVersionInfo(
      releases,
      process.platform,
      process.arch,
      shim.isPortable(),
    ...
    if (compareVersions(release.version, packageInfo.version) <= 0) {
      ...
      const shouldSkip =
        checkInBackground_ && (await isSkippedVersion(release.version));

      if (shouldSkip) {
        logger.info(
          "Not displaying notification because version has been skipped"
        );
      } else {
        ...
        const truncateReleaseNotes =
          fullReleaseNotes.length > MAX_RELEASE_NOTES_LENGTH;
        const releaseNotes = truncateReleaseNotes
          ? truncateText(fullReleaseNotes, MAX_RELEASE_NOTES_LENGTH)
          : fullReleaseNotes;

        const newVersionString = release.prerelease
          ? _("%s (pre-release)", release.version)
          : release.version;
        ...
        if (buttonIndex === 0) {
          ...
          logger.info("Start applying");
          ipcRenderer.send("runUpdateBackground");
