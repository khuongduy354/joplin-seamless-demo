...
import runUpdateBackground from "./runUpdateBackground";
...
export default class ElectronAppWrapper {
  ...
  public createWindow() {
    ...
    const stateOptions: any = {
      ...
      defaultHeight: Math.round(
        0.8 * screen.getPrimaryDisplay().workArea.height
      ),
    ...
    if (!screen.getDisplayMatching(this.win_.getBounds())) {
      const { width: windowWidth, height: windowHeight } =
        this.win_.getBounds();
      const { width: primaryDisplayWidth, height: primaryDisplayHeight } =
        screen.getPrimaryDisplay().workArea;
      this.win_.setPosition(
        primaryDisplayWidth / 2 - windowWidth,
        primaryDisplayHeight / 2 - windowHeight
      );
    }
    ...
    ipcMain.on("runUpdateBackground", (_event: any) => {
      runUpdateBackground(async function () {
        await dialog.showMessageBox({
          message: _("Starting Installation in Background"),
          buttons: [_("Okay")],
          cancelId: 0,
        });
      });
    ...
    ipcMain.on(
      ...
      (_event: any, message: string, args: any) => {
    ...
    ipcMain.on("pluginMessage", (_event: any, message: PluginMessage) => {
      try {
        ...
        if (message.target === "plugin") {
          ...
          if (!win) {
            this.logger().error(
              `Trying to send IPC message to non-existing plugin window: ${message.pluginId}`
            );
            return;
          }
  ...
  public createTray(contextMenu: any) {
    try {
      this.tray_ = new Tray(
        `${this.buildDir()}/icons/${this.trayIconFilename_()}`
      );
